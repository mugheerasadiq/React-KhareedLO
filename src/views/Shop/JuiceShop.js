...
const JuiceShop = ({ getAllProducts, productData = [] }) => {
  ...
  useEffect(() => {
    const juices = productData.filter(
      (item) => item.category.name.toLowerCase() === "juice"
    );
    setAllJuices(juices);
    calculateFreshness(juices);
  }, [productData]);
          ...
          <div className="row justify-content-center">
            <div className="col-md-10 mb-5 text-center">
              <ul className="product-category">
                ...
                <li>
                  <Link to="/juiceShop" className="active">
                    Juice
                  </Link>
                </li>
                <li>
...
export default connect(mapStateToProps, mapDispatchToProps)(JuiceShop);
